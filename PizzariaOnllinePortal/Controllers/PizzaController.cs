...
namespace PizzariaOnlinePortal.Controllers
{
    ...
    public class PizzaController : Controller
    {
        ...
        public PizzaController(IWebHostEnvironment hostingEnvironment, IConfiguration configuration)
        {
        ...
        public async Task<ViewResult> Index()
        {
            ...
            {
                listPizza = await GetListOfPizza();
            }
        ...
        [HttpGet]
        public ViewResult CreateIngredients()
        {
            return View();
        }
        [HttpPost]
        public async Task<IActionResult> createIngredients(CreateIngredientViewModel model)
        {
            if (ModelState.IsValid)
            {
                string uniqeFileName = ProcessUploadedFileForIngredient(model);
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    string endpoint = apiBaseUrl + "/Ingredient/AddIngredient";
                    //List<Pizza> Pizzas = null;
                    using (var Response = await client.PostAsync(endpoint, content))
                    {
                        // This is list view of all pizza's from database. call the API to get the result                        
                        ...
                        {
                            ModelState.Clear();
                            ModelState.AddModelError(string.Empty, "Failed to Save AddIngredient  to database");
                            return View();
                        }
                    }
                }
        ...
        private string ProcessUploadedFileForIngredient(CreateIngredientViewModel model)
        {
            string uniqeFileName = null;
            if (model.Photo != null)
            {
